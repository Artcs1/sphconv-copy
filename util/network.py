...
def load_network(layer="1", network="faster-rcnn", silence=True):
    proto_root = os.path.join(SphConv_ROOT, "prototxt")
    model_proto = os.path.join(proto_root, 'superpoint_layers', '{0}{1}.prototxt'.format(network, layer))
    
    if network == "superpoint":
        model_bin = os.path.join(cfg.caffe_root, "models/superpoint/superpoint.caffemodel")
    elif network == "vgg16":
